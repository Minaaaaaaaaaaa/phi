...
function load() {
  try {
    const raw = localStorage.getItem('phi-state');
    if (raw) {
      const parsed = JSON.parse(raw);
      state.projects = parsed.projects || [];
      state.todayTasks = parsed.todayTasks || [];
      state.todayDate = parsed.todayDate || null;
      if (typeof parsed.pomoWorkMinutes === 'number') {
        pomo.workMinutes = parsed.pomoWorkMinutes;
  ...
  // Backfill cumulative-completion fields for projects created before they existed.
  ...
  // On a day change, keep incomplete tasks (they carry over) and drop completed ones
  ...
  // Load sessions
  try {
    const raw = localStorage.getItem('pomodoroSessions');
    phiSessions = raw ? JSON.parse(raw) : [];
  } catch (e) {
    phiSessions = [];
  }
}
...
function saveSessions() {
  localStorage.setItem('pomodoroSessions', JSON.stringify(phiSessions));
}

function save() {
  localStorage.setItem('phi-state', JSON.stringify({
    projects: state.projects,
    todayTasks: state.todayTasks,
    todayDate: state.todayDate,
    pomoWorkMinutes: pomo.workMinutes
  }));
}
...
function projectTimeStats(project) {
  const legacy = project.sessions || [];
  const newer = phiSessions.filter(s => s.projectId === project.id);
  const all = legacy.concat(newer);
...
function projectTodayMinutes(project) {
  ...
  const all = (project.sessions || []).concat(phiSessions.filter(s => s.projectId === project.id));
  return all
...
function renderProjects() {
  ...
  for (const project of state.projects) {
    ...
    card.innerHTML = `
      ...
      ${project.tasks.length > 6 ? `<button class="expand-btn" data-expand="${project.id}">${expanded ? 'show less' : 'show more'}</button>` : ''}`;
    content.appendChild(card);
  }
...
function loadDayStart() {
  const v = localStorage.getItem('weeklyDayStart');
  if (v !== null) {
    const parsed = parseInt(v);
    if (!isNaN(parsed) && parsed >= 0 && parsed <= 23) dayStartHour = parsed;
  }
}
...
function init() {
  load();
  // Day-start picker is removed; always start the grid at the hardcoded dayStartHour (08).
